...
/*   Updated: 2024/11/12 12:12:57 by sabartho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mandelbrot_burning_ship(t_mlx *app, t_complex *cpl)
{
	...
	cpl->cx = ((app->x_min + ((float)cpl->x / WIDTH)
				* (app->x_max - app->x_min)) + app->x_move) * ar_width;
	cpl->cy = ((app->y_min + ((float)cpl->y / HEIGHT)
				* (app->y_max - app->y_min)) + app->y_move) * ar_height;
}
...
void	choose_fractol(t_complex *cpl, t_mlx *app)
{
	if (app->id == 0)
		julia(app, cpl);
	else if (app->id == 1 || app->id == 2)
		mandelbrot_burning_ship(app, cpl);
}
